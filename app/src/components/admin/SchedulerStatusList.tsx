  ...
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-border bg-surface-container-lowest p-4 shadow-sm">
      ...
      <div className="flex flex-col gap-3">
        ...
        {status === "error" ? (
          <div className="rounded-2xl border border-severity-critical/30 bg-severity-critical-container p-4 text-sm text-severity-critical">
            Não foi possível carregar o status dos schedulers agora.
          </div>
        ) : null}
        {schedulers.map((scheduler) => (
          <article key={scheduler.id} className="rounded-2xl border border-border bg-muted/20 p-4 shadow-xs">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-xl bg-surface-container-lowest border border-border/60 p-3">
            ...
            {scheduler.lastErrorMessage ? (
              <p className="mt-3 rounded-xl bg-severity-critical-container p-3 text-sm text-severity-critical">
                {scheduler.lastErrorMessage}
              </p>
            ) : null}
          </article>
